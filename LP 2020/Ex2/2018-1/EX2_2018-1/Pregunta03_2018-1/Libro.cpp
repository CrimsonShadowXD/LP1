...
void Libro::operator=(const class Libro& libro) {
    ...
    SetNVend(libro.nVend);
    SetMonto(libro.monto);
}
...
void Libro::imprime(ofstream& arch) {
    ...
    //datos del libro
    list<Autor>::iterator it = autores.begin();
    arch<<left<<setw(10)<<codigo<<setw(62)<<titulo;
    
    //datos de los autores
    if (autores.size()>0) arch<<*it;
    else arch<<left<<setw(36)<<" ";
      
    //datos relacionado a costos y ventas del libro
    arch<<setw(20)<<precio<<setw(10)<<nVend<<setw(6)<<monto<<endl;
